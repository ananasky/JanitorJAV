...
def test_pages_render(tmp_path: Path) -> None:
    ...
    assert 'id="path-query"' in response.text
    ...
    assert 'id="operation-progress"' in response.text
    assert 'watchQuarantineJob' in response.text
    assert 'quarantine-ready-job' in response.text
    assert 'id="operation-progress-errors"' in response.text
